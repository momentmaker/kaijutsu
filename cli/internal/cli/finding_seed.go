...
func seedDevGate(cmd *cobra.Command) error {
	...
	// Walk parent chain looking for the --dev persistent flag. The
	// flag is registered on the `finding` group cmd, but cobra also
	// surfaces it via inherited flags on this subcmd's flag set —
	// safer to check both since parent layout could shift.
	for c := cmd; c != nil; c = c.Parent() {
		if f := c.PersistentFlags().Lookup("dev"); f != nil && f.Value.String() == "true" {
			return nil
		}
	}
...
func seedActioned(s *findings.Store, provider, persona, preset, fp, lens string, accepts, dismisses int) (int, error) {
	...
	written := 0
	runID := fmt.Sprintf("seed-%d", now.UnixNano())
	for i := 0; i < accepts; i++ {
		if _, err := stmt.Exec(

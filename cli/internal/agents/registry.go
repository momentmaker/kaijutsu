...
var (
	...
	defaultAntigravity = &antigravityDriver{}
)
...
//   - DriverCLI       → singleton claude/codex/antigravity OR generic cliDriver
...
func BuildDriver(p *Provider) (AgentDriver, error) {
	...
	case DriverCLI:
		...
		// p.Cmd. Only claude/codex/antigravity are mappable; user-declared
		...
		return nil, fmt.Errorf("cli driver for provider %q not implemented (Stage 4 adds generic cmd-based cli drivers; for now, name must be claude|codex|antigravity)", p.Name)
	case DriverHTTP:
		if p.BaseURL == "" {
	...
	case DriverCLICompat:
		if p.BaseCLI == "" {
			return nil, fmt.Errorf("provider %q (cli-compat): base_cli is required (claude | codex | antigravity)", p.Name)
		}
		base := For(p.BaseCLI)
		if base == nil {
			return nil, fmt.Errorf("provider %q (cli-compat): base_cli %q is not a known cli driver (allowed: claude, codex, antigravity)", p.Name, p.BaseCLI)
		}

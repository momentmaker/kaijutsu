...
> **Status: alpha.** The `jutsu` CLI ships end-to-end skill management (init / install / list / remove / upgrade / lint / search / info / publish), multi-agent swarm (driver abstraction + `agents.yaml` + personas), quality fingerprinting (local SQLite findings store + confidence-weighted synthesizer), and a Preset SDK (`swarm.yaml` per project / per user). Public catalog at [kaijutsu.dev](https://kaijutsu.dev). Install paths, skill schema, command surface stable for v0.x. Latest release notes: [`CHANGELOG.md`](./CHANGELOG.md). Security model: [`SECURITY.md`](./SECURITY.md).

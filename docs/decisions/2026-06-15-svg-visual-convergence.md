...
- **Repo-wide gap (out of scope here):** CI does not validate any `skill.yaml` against `schemas/skill.schema.json` (lint-skills only compiles the schemas and checks for a sibling `SKILL.md`), and while the Go loader *parses* `shared`/`permissions`/`deps`/`trust`, `Validate()` does not deeply check their values (only `name`/`version`/`license`/`layout`/`agents`/`hooks`). A maintainer may want a repo-level `ajv-validate-skills` step.

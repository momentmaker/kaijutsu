...
| Cache key for non-diff inputs | **First 12 lowercase hex chars of SHA256(preset-name ‖ 0x00 ‖ canonicalized_input)** — the 0x00 (NUL byte) separator prevents cross-preset collisions of the form `preset="ab"+input="cd"` vs `preset="abcd"+input="" `. Applies to both `InputFiles` and `InputPrompt` and to all non-diff presets (doc-review, brainstorm, refactor-plan, file-mode security-audit). Canonicalization rules: (a) `InputFiles` — sort paths lexically before concat, prefix each section with `--- <path>\n` so file order doesn't change the hash; (b) `InputPrompt` — `strings.TrimSpace` + collapse runs of internal whitespace to single space + Unicode NFC normalize. Truncation to 12 chars = 48 bits of entropy; birthday bound for ~10k cache entries is ~3×10⁻⁵ — fine for single-repo cache history. On collision the second run overwrites the first (last-write-wins, no error); concurrent writes are not handled because two `jutsu swarm` invocations against the same input + same preset is itself a user error. Each preset's stage acceptance section MUST reference these rules verbatim, NOT re-state simpler ones. |
| Input byte caps per kind | `InputDiff`: 200 KB (matches gemini -p stdin fallback threshold + leaves room for prompt overhead). `InputFiles`: 200 KB total across all paths. `InputPrompt`: 8 KB. Over-cap aborts with kind-specific hint: InputDiff → "narrow PR scope or split into multiple reviews"; InputFiles → "split files into multiple runs"; InputPrompt → "shorten the prompt". Phase 2 ships fixed defaults; per-preset override via skill metadata is deferred to Phase 3 (would require a `MaxBytes int` field on the Preset struct + plumbing through ResolveInput). Tracked as Open Question 5. |
...
| `--post-comment` for non-PR presets | **pr-review-only flag.** Two-layer enforcement (matches Phase-2 implementation): (a) non-PR subcommands (doc-review, brainstorm, refactor-plan, files-mode security-audit) do NOT register the `--post-comment` flag — passing it errors with cobra's "unknown flag". (b) PR-mode subcommands (pr-review, PR-mode security-audit) accept the flag but emit "warns + skips" if the resolved input has no PR (e.g., `--diff-from-branch` mode). Net: user CAN'T accidentally post to a non-existent PR; non-PR presets stick to print + cache. Generic `--output-to file\|gist\|issue\|slack` deferred to Phase 3. |
| doc-review section addressing | **file + line_range** (same schema fields as pr-review, NOT a new section/paragraph schema). Per-agent prompts ask the model to MENTION section names in its `reasoning` text ("In **Acceptance Criteria** at line 42, ..."), but structured fields stay file:line for disagreement-table parity with pr-review. Markdown-aware section/paragraph schema deferred to Phase 3 (would require a markdown parser + assembler refactor; out of scope for the flywheel MVP). Stage 3 acceptance reflects this. |
...
| Acceptance criteria style per stage | Two flavors. **Mechanical**: Stage 1, 2, 4, 7 deliverables are testable code (registry lookup, byte-cap enforcement, skill migration shape, mutex-flag check). Their acceptance lines describe specific tests that PASS or FAIL. **Manual-review gate**: Stages 3, 5, 6 deliverables include synthesis-style outputs (markdown reviews, brainstorm option lists, refactor step plans) whose quality is judged by the human running them. Their acceptance lines describe what the output SHOULD contain (e.g., "ordered step list with risk column") and a smoke-test invocation that confirms the preset RUNS, but quality verification stays manual. Reviewer-tooling that flags "this acceptance isn't mechanically testable" should ignore the manual-review-gate stages. |
...
Severity vocab: per the locked-decisions row above (4 levels:
`recommended | alternative | risky | speculative`). Output
assembler: ordered step list with risk column + estimated effort.
